...
export function getColorFromProps({ color, ...props }) {
  if (!!color) return [color, props]
  color = SCALE.find((key) => !!props[key])
  return [color, omit(SCALE, props)]
}

export function useSmartColor(props, defaultValue) {
  const getColor = useGetColor()
  const [color, formattedProps] = getColorFromProps(props)
  return [getColor(color || defaultValue), formattedProps]
}
